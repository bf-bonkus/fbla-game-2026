...
func _on_feed_pressed() -> void:
	ReactorCore.actors[&"pet"].context[&"hunger"] += 50
	Transactions.transact("Fly", -0.15)
	ReactorCore.actors[&"pet"].query()

	var pet: Node = ReactorCore.actors[&"pet"].get_parent()
	pet.find_child("HungerTimer").start()
	pet.find_child("FlyCrumbs").emitting = true
